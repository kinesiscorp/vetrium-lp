...
import { NAV_SECTIONS, type SectionId } from "@/lib/proposal-frog/content";

/** Barra fixa com progresso de leitura + índice de seções. Existe porque a
 *  proposta agora é uma página contínua, não folhas separadas — sem isso,
 *  quem abre perde a noção de "quanto falta" e de como pular direto pro
 *  investimento. */
  ...
  return (
    <header data-no-print className="fixed inset-x-0 top-0 z-50">
      <div className="flex items-center justify-between border-b border-line bg-bg/85 px-5 py-3 backdrop-blur-md sm:px-8">
        <a href="#capa" className="text-[15px] font-semibold tracking-tight">
          Vetrium
        </a>
                  ...
                  {NAV_SECTIONS.map((s) => (
                    <li key={s.id}>
                      <a
                        href={`#${s.id}`}
                        onClick={(e) => {
                          // Fechar o menu no mesmo clique remove este link do DOM
                          // antes do navegador concluir o "scroll to fragment"
                          // nativo — a rolagem some. Rolamos manualmente.
                          // (Nada de history.pushState aqui: o App Router do
                          // Next intercepta a History API, e disparar pushState
                          // à mão faz o router dele "restaurar" o scroll pra 0.)
                          e.preventDefault();
                          setOpen(false);
